// The first element, element 1, (or 0 depending upon which counting scheme is used) is assigned nodes 1 - 6 (0 - 5).
// All lower elements in the bottom row are assigned nodes 2 - 6 (1 - 5).
// All upper elements not in the front column are assigned node 4 (3).
// All lower elements not on the bottom row and all upper elements in the front column are assigned nodes 2,4,5 (1,3,4).
...
// 			  E_SDM Str_Old
  ...
  // Stress variables
  E_SDM El_Dep_Str, Str_Elxy; // El_Str, 
  E_SDM Str_Gdsf;
...
// 	  El_Str(3,i) = Str_Old(Inod,2);
    ...
    // minus 1 for C++ indexing, example, Ele 2 given by Ne = 1.
    // If Ne an lower element in the first row.
    else if((Ne % 2 == 0) && (Ne <= 2 * NX - 2))
    {
      for (int j = 1; j <= Str_Npe - 1; j++)  // loop over nodes 2 - 6 ( 1 - 5) for the first row
      {
	Vel_Inod = Vel_Nod(Ne,j) - 1;
...
// 	  El_Str(3,i) = Str_Old(Inod,2);
    ...
    // If Ne is an upper element on the front or a lower element
    // not in the first row.
    else if((Ne % (2 * NX) == 1) || (Ne % 2 == 0))
    {
...
// 	}
    ...
    // If Ne is a lupper element not in th efirst row or column
    else// if(Ne % 2 == 1) // ((Ne + 1) % (2 * NX) != 2) || 
    {
